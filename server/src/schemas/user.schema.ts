...
export const createUserSchema = registry.register('CreateUser', z.object({
    body: z.object({
        ...
        validFrom: z.string().date('Valid from date must use YYYY-MM-DD').optional(),
        cardExpires: z.string().date('Card expiration date must use YYYY-MM-DD').optional(),
        migratoryStatus: z.string().trim().min(1, 'Migratory status is required').optional(),
        receivedDate: z.string().date('Received date must use YYYY-MM-DD').optional(),
        deadline: z.string().date('Deadline must use YYYY-MM-DD').optional(),
...
export const updateUserSchema = registry.register('UpdateUser', z.object({
    body: z.object({
        ...
        sex: z.enum(['M', 'F']).or(z.literal('')).transform((value) => value || undefined).optional(),
        validFrom: optionalDate,
        cardExpires: optionalDate,
